...
public class Parser {
	...
	void Declarations () throws UnexpectedTokenException{
		Avanza();
		//Verifica que tipo de token es el que se utiliza, si no se sale del método.
		if(tokenCode == Int || tokenCode == Float){
			...
			Come(Semi);
			//Agrega al arbol de declaraciones una nueva declaración.
			declaracion = new DefaultMutableTreeNode(type);
			declaracion.add(new DefaultMutableTreeNode(identifier));
			//Añade el nodo declaracion al arbol declaraciones.
			declaraciones.add(declaracion);
			//Llamado recursivo.
			Declarations();
		}
	}
	//Método para la creacion y verificacion de los estatutos, regresa un estatuto.
	Statement Statements() throws UnexpectedTokenException{
		switch(tokenCode){
			...
			case If:
				...
				//Asgina un nodo con la expresión.
				...
				//Añade el nodo statement al arbol statements.
			...
			case Print:
				Come(Print);
				//guarda una expresión de tipo comparacion
				Comparation e = (Comparation) Expresion();
				Avanza();
				//Guarda en nodos de arbol la información.
	...
	//Método para realizar la parte de la expression de igualdad.
